...
jQuery(document).ready(function ($) {
  ...
  if (!localStorage.values) {
    ...
    localStorage.values = JSON.stringify({
      xArray: xArray,
      xArrayForCalc: xArrayForCalc,
      yArray: yArray,
      modelCoefficients: testModelCoefficients
    });
  } else {
    ...
    xArray = values.xArray;
    xArrayForCalc = values.xArrayForCalc;
    testModelCoefficients = values.modelCoefficients;
    yArray = values.yArray;
  }
  ...
  (function startLogic(Fisher, xArrayForCalc, yArray) {
    var S4 = solveWithRegressionLogic(xArrayForCalc, yArray);
    var F = S4.fisher;
    var R4 = S4.r;
    if (xArrayForCalc.length === 4 && F < Fisher) {
      alert('model is considerable with ' + xArrayForCalc.length + ' vars');

      return;
    }

    if (xArrayForCalc.length === 4) {
      var s4 = [];

      var x4 = xArrayForCalc.slice(0, 3);
      s4.unshift(solveWithRegressionLogic(x4, yArray));

      var x3 = xArrayForCalc.slice(0, 2).concat(xArrayForCalc.slice(-1));
      s4.unshift(solveWithRegressionLogic(x3, yArray));

      var x2 = xArrayForCalc.slice(0, 1).concat(xArrayForCalc.slice(2, 4));
      s4.unshift(solveWithRegressionLogic(x2, yArray));

      var x1 = xArrayForCalc.slice(1);
      s4.unshift(solveWithRegressionLogic(x1, yArray));

      var maxDiff = Math.abs(1 - s4[0].r);
      var ind = 0;
      for (var i = 0; i < s4.length; i++) {
        if (maxDiff > Math.abs(1 - s4[i].r)) {
          maxDiff = Math.abs(1 - s4[i].r);
          ind = i;
        }
      }
      ...
      if (checkWithFisher(R4, R, 20, 3) > Fisher) {
        alert("X" + (ind + 1) + " nshanakali e");

        //var answerText = 'Answer: Y = ';
        //
        //for (i = 0; i < xArrayForCalc.length + 1; i++) {
  ...
  })(3, xArrayForCalc, yArray);

  function checkWithFisher(Rall, Rw, n, k) {
    return ( (Rall - Rw)*(n - k - 1) / (1 - Rall) );
  }

  // main Regression Logic
  function solveWithRegressionLogic(xArrayForCalc, yArray) {
    var xArray = xArrayForCalc[0].map(function(col, i) {
      return xArrayForCalc.map(function(row) {
        return row[i];
      });
    });
    ...
    var Gxj = [];
    for (i = 0; i < xArrayForCalc.length; i++) {
    ...
    for (i = 0; i < xArrayForCalc.length; i++) {
      var dummy = [];
      for (j = 0; j < xArrayForCalc.length; j++) {
        dummy.push(i === j ? 1 : 0);
      }
    ...
    for (j = 0; j < xArrayForCalc.length; j++) {
      for (var l = 0; l < xArrayForCalc.length; l++) {
        if (l !== j) {
          dummyVar = 0;
    ...
    var msR = ssR/xArrayForCalc.length;

    console.log('MS(R): ', msR);

    var G2 = ssE/(n - xArrayForCalc.length -1);
